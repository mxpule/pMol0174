...
pMolTraceAbi::pMolTraceAbi(const QString &fileName)
{
  DataCount[0] = 9;
  DataCount[1] = 10;
  DataCount[2] = 11;
...
//    badnessHighlighted = true;
  ...
  {
    qDebug() << data->NPoints << data->NBases;
    char       *base = data->base;        /* Array of length `NBases' */
    uint_2     *basePos = data->basePos;

  }
};


void pMolTraceAbi::resizeEvent(QResizeEvent *event)
{
   reSized = true;			
   QAbstractScrollArea::resizeEvent(event);
}

void pMolTraceAbi::paintLine(QPainter* painter, int x, int y, TRACE* trace, float scale)
{
  int y0 = y + drawTraceHeight - ((int) (*(trace))/scale);
  int y1 = y + drawTraceHeight - ((int) (*(trace+1))/scale);
  painter->drawLine(x,y0,x+1,y1);

};


void pMolTraceAbi::paintEvent(QPaintEvent *)
{
  int w = width()-40;		//distance from the edge of the window

  QPainter painter(viewport());
    TRACE      *traceA = data->traceA;      /* Array of length `NPoints' */
    TRACE      *traceC = data->traceC;      /* Array of length `NPoints' */
    TRACE      *traceG = data->traceG;      /* Array of length `NPoints' */
    TRACE      *traceT = data->traceT;      /* Array of length `NPoints' */
    TRACE       maxTraceVal; /* The maximal value in any trace */

   float scale = (float) data->maxTraceVal /  drawTraceHeight;
  ...
  if (reSized)
  {
    ...
    int totalHeight = ((int) ceil((double) data->NPoints / w));
    qDebug() << "totalHeight" << data->NPoints << w << totalHeight;
    scrollbar->setMaximum(totalHeight * drawTraceHeight - height() + 10);	
    scrollbar->setPageStep(drawTraceHeight);
    reSized = false;
  };

  int i = 0;
  int y=-scrollbar->sliderPosition();
  
  while (i<data->NPoints)
  {
    if (y+drawTraceHeight>=0)
    for (int x=0; x<w; x++)
    {
      if (x+i > data->NPoints) return;

      painter.setPen(QColor(255,0,0,255));
      paintLine(&painter, x, y, traceA+i+x, scale);

      painter.setPen(QColor(0,255,0,255));
      paintLine(&painter, x, y, traceC+i+x, scale);

      painter.setPen(QColor(0,0,255,255));
      paintLine(&painter, x, y, traceG+i+x, scale);

      painter.setPen(QColor(0,0,0,255));
      paintLine(&painter, x, y, traceT+i+x, scale);
   }
   i+=w;
   y+= drawTraceHeight;
   if (y+drawTraceHeight>height()) return;
  };
};
...
float pMolTraceAbi::int_to_float(int in)
/*
...
int pMolTraceAbi::be_read_int_4(FILE *fp, unsigned int *i4)
{
    uint_4 i;
 ...
 */
int pMolTraceAbi::be_read_int_1(FILE *fp, uint_1 *i1)
{
 ...
 */
int pMolTraceAbi::be_read_int_2(FILE *fp, uint_2 *i2)
{
    uint_2 i;
...
int pMolTraceAbi::read_sections(int new_sec) {
...
//-----------------
Read *pMolTraceAbi::read_allocate(int num_points, int num_bases) {
    Read *seq = NULLRead;
 ...
 */
void pMolTraceAbi::read_deallocate(Read *read)
{
    if (read == NULLRead)
	return;
...
int pMolTraceAbi::dump_labels(FILE *fp, off_t indexO) 
{
    off_t entryNum = -1;
    unsigned int entryLabel, entryLw1;
 ...
 */
int pMolTraceAbi::getABIIndexEntryLW(FILE *fp, off_t indexO,
 ...
 * The result is 0 for failure, or index offset for success.
 */
int pMolTraceAbi::getABIIndexEntrySW(FILE *fp, off_t indexO,
 ...
 */
int pMolTraceAbi::getABIIndexOffset(FILE *fp, uint_4 *indexO) {
    uint_4 magic;
 ...
 */
int pMolTraceAbi::getABIString(FILE *fp, off_t indexO, uint_4 label, uint_4 count,
		 char *string) {
    uint_4 off;
    uint_4 len;
    uint_2 type;
...
void pMolTraceAbi::replace_nl(char *string) {
    char *cp;
 ...
 * was more to come).
 */
int pMolTraceAbi::getABIint1(FILE *fp, off_t indexO, uint_4 label, uint_4 count,
 ...
 * was more to come).
 */
int pMolTraceAbi::getABIint2(FILE *fp, off_t indexO, uint_4 label, uint_4 count,
 ...
 * was more to come).
 */
int pMolTraceAbi::getABIint4(FILE *fp, off_t indexO, uint_4 label, uint_4 count,
 ...
 */
void pMolTraceAbi::abi_set_data_counts(int f, int w, int o, int _) {
    DataCount[0] = f;
    DataCount[1] = w;
    DataCount[2] = o;
    DataCount[3] = _;
 ...
 */
void pMolTraceAbi::abi_reset_data_counts(void) {
    DataCount[0] = 9;
    DataCount[1] = 10;
    DataCount[2] = 11;
    DataCount[3] = 12;
 ...
 *   NULLRead	- Failure.
 */
Read* pMolTraceAbi::fread_abi(FILE *fp) {
    Read *read = NULLRead;
    int i;
 ...
 *   NULLRead	- Failure.
 */
Read* pMolTraceAbi::read_abi(char *fn) {
    Read *read;
    FILE *fp;
...
pMolTraceAbi* pMolTraceAbi::create(pMolKernelInterface* interface, pMolCmd* cmd)
{
  QString error;
  ...
  if (list->checkParameters(pMolObject::STRING, error))
  {
    ...
    qDebug() << "in trace" << string->string;
    pMolTraceAbi* trace = new pMolTraceAbi(string->string);
    trace->show();
